...
infile := DATASET([{'A.B.C A'},{'A.B.C A'},
                   {'A.B.C A'},{'A.B.C A'}],{STRING20 rawnumber});
...
OutRec NormThem(FlatRec L, INTEGER C) := TRANSFORM
	...
	SELF.val := REGEXFINDSET(expr2,(STRING)L.term)[C]+(' '+Std.Str.SplitWords((string)L.term,'.')[C]);
//letter:=Std.Str.SplitWords((string)L.term,'.');
	//SELF.term := Std.Str.SplitWords((string)SELF.val,' ')[C]);
	
	SELF.len:=if(L.term=REGEXFINDSET(expr2,(STRING)L.term)[C],L.len,length(Std.Str.SplitWords((string)L.term,'.')[C]));
	SELF.lenText:=if(L.term=REGEXFINDSET(expr2,(STRING)L.term)[C],L.len,length(Std.Str.SplitWords((string)L.term,'.')[C]));
	
	
 SELF.kwp:=L.kwp+C;
// SELF.keywords:=if(length(LEFT.Values[COUNTER].val)=1,1,LEFT.keywords)
	//									,SELF.lentext:=length(LEFT.Values[COUNTER].val),SELF.typterm:=if(length(LEFT.Values[COUNTER].val)=1,1,LEFT.typterm);
    //                SELF := LEFT,
			//							 ));
											
	
	//SELF.len:=length(L.term);
	SELF := L;
 
	
END;
ChildRecs := NORMALIZE(rawPostings,LEFT.keywords,NormThem(LEFT,COUNTER));//#of keywords  rawPostings.keywords
//ChildRecs := NORMALIZE(rawPostings,LEFT.keywords,NormThem(LEFT,COUNTER),SELF.len:=length(LEFT.term));
 /*res:=NORMALIZE(NestedDS,COUNT(LEFT.Values),
          TRANSFORM(OutRec,
                    SELF.val := LEFT.Values[COUNTER].val,Self.term:=LEFT.Values[COUNTER].val,SELF.len:=length(LEFT.Values[COUNTER].val),SELF.kwp:=LEFT.kwp+COUNTER,SELF.keywords:=if(length(LEFT.Values[COUNTER].val)=1,1,LEFT.keywords)
										,SELF.lentext:=length(LEFT.Values[COUNTER].val),SELF.typterm:=if(length(LEFT.Values[COUNTER].val)=1,1,LEFT.typterm);
...
out := PARSE(ChildRecs, val,r,ps1,Best,MANY, NOT MATCHED);
//output(outt);
 ...
 PATTERN expr7 :=PATTERN('[a-zA-Z][.][a-zA-Z]*[.][a-zA-Z]*[.]*[a-zA-Z]*[\' \'][a-zA-Z]+');
//PATTERN expr4 :=PATTERN('[' ']');
//PATTERN expr5 :=PATTERN('[\' \'] [a-zA-Z]');
TOKEN JustAWord := expr7;
RULE r   := JustAWord ;	
ps1 := { 
